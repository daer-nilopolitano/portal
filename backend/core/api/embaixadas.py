...
from ..models import Embaixada, HorarioReuniao, Igreja, Pessoa
...
class EmbaixadaPublicaOut(Schema):
    ...
    data de nascimento) que existe em Pessoa."""
    ...
    def resolve_conselheiros_nomes(obj: Embaixada) -> list[str]:
        ...
        # pessoa apareça nos dois (ex.: cadastro feito de forma redundante).
        ...
        for pessoa in obj.conselheiros.all():
            nomes_vistos.setdefault(pessoa.id, pessoa.nome)
...
def criar_embaixada(request, payload: EmbaixadaIn):
    ...
    if payload.conselheiro_responsavel_id:
        conselheiro_responsavel = get_object_or_404(Pessoa, pk=payload.conselheiro_responsavel_id)
    ...
    if payload.conselheiro_ids:
        embaixada.conselheiros.set(Pessoa.objects.filter(pk__in=payload.conselheiro_ids))
...
def atualizar_embaixada(request, embaixada_id: int, payload: EmbaixadaUpdate):
    ...
    if conselheiro_ids is not None:
        embaixada.conselheiros.set(Pessoa.objects.filter(pk__in=conselheiro_ids))
